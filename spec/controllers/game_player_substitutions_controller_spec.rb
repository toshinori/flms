...
describe GamePlayerSubstitutionsController do
  ...
  describe "GET 'new'" do
    context 'when send valid params' do
      ...
      describe 'assings' do
        it { should assign_to(:game_player_substitution).with_kind_of(GamePlayerSubstitution) }
  ...
  describe "POST 'create'" do
    context 'when send valid params' do
      before do
        @send_params = attributes_for(:game_player_substitution_base)
        @player = GameMember.find(@send_params[:game_member_id])
        @count = GamePlayerSubstitution.find_all_by_game_member_id(@player.id).size
        xhr :post, :create, { game_player_substitution: @send_params, game_team_id: @player.team.id }, @valid_session
      end

      it 'record count 1 up' do
        substitution_count = GamePlayerSubstitution.find_all_by_game_member_id(@player.id).size
        substitution_count.should === (@count + 1)
      end
    ...
    context 'when send invalid params' do
      before do
        ...
        xhr :post, :create, { game_player_substitution: @send_params, game_team_id: @game_team_id }, @valid_session
      end
  ...
  describe "DELETE 'destroy'" do
    context 'when send valid params' do
      before do
        @target = create(:game_player_substitution_base)
        @count = GamePlayerSubstitution.count
      ...
      it "record count 1 down" do
        GamePlayerSubstitution.count.should == (@count - 1)
      end
    end

    context 'when send invalid params' do
      before do
        xhr :delete, :destroy, {id: (GamePlayerSubstitution.maximum(:id).to_i + 1)}, @valid_session
      end
      describe 'reponse' do
