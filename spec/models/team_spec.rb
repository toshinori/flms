...
describe Team do
  ...
  describe 'players' do
    context 'when add 5 players' do
      ...
      subject { @team }
      let!(:players_count) { subject.members.find_all_by_member_type(Constants.member_type[:player]).count }
      its('players.count') { should == players_count}
      it { subject.players.all {|p| p.member_type  == Constants.member_type[:player]}.should be_true }
    end
  end
  ...
  describe 'managers' do
    context 'when add 5 managers' do
      ...
      subject { @team }
      let!(:managers_count) { subject.members.find_all_by_member_type(Constants.member_type[:manager]).count }
      its('managers.count') { should == managers_count }
      it { subject.managers.all {|p| p.member_type  == Constants.member_type[:manager]}.should be_true }
    end
  end
